...
setMethod("plot", signature(x = "ea_data", y = "missing"),
      ...
      indicator = {
        ...
        p <- p +
          ...
          ggplot2::geom_point(ggplot2::aes(color = .data$point_color), ...) +
          ggplot2::geom_hline(
            yintercept = long_mean, color = "darkgreen",
            linetype = "dashed", linewidth = 1
          ) +
          ggplot2::geom_hline(
            yintercept = long_mean + sd_threshold * sd_value, color = "darkgreen",
      ...
      indicator_ref = {
        ...
        p <- p +
          ggplot2::geom_path(color = "black", linewidth = 0.8, ...) +
          ggplot2::geom_hline(
            yintercept = ref_mean, color = "darkgreen",
            linetype = "dashed", linewidth = 1
          ) +
          ggplot2::geom_hline(
            yintercept = ref_mean + sd_threshold * ref_sd, color = "darkgreen",
            linewidth = 0.8
          ) + # +SD threshold
          ggplot2::geom_hline(
            yintercept = ref_mean - sd_threshold * ref_sd, color = "darkgreen",
      ...
      diversity = {
        ...
        p <- p +
          ggplot2::geom_hline(
            yintercept = diversity_mean, color = mean_line_color,
            linetype = "dashed", linewidth = 1
          )
      ...
      temperature_regime = {
        ...
        p <- p +
          ...
          ggplot2::geom_hline(
            yintercept = temp_mean, color = "black",
            linetype = "solid", linewidth = 0.8
          ) +
          ggplot2::geom_hline(
            yintercept = temp_mean + c(-1, 1) * regime_threshold * temp_sd,
            color = "grey50", linetype = "dashed", alpha = 0.7
          )
        ...
        labs$subtitle <- paste0(
          labs$subtitle,
          sprintf(
            " | Mean: %.2f C, Regime threshold: +/-%.1f SD",
            temp_mean, regime_threshold
          )
        )
        p
      ...
      nao_enhanced = {
        ...
        p <- p +
          ...
          ggplot2::scale_fill_manual(
            values = c(
              "strong_negative" = "#053061", "negative" = "#4393c3",
              "positive" = "#d6604d", "strong_positive" = "#67001f"
            ),
            name = "NAO Phase",
            labels = c("Strong Negative", "Negative", "Positive", "Strong Positive")
