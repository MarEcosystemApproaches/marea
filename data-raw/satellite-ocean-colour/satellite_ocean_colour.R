...
metrics <- list(
  t_start = list(
    data_type = "Day of year of the start of the spring phytoplankton bloom",
    units = "day of year"
  ),
  t_duration = list(
    data_type = "Duration of the spring phytoplankton bloom",
    units = "days"
  ),
  amplitude_real = list(
    data_type = "Maximum concentration during the spring phytoplankton bloom period",
    units = "mg/m3"
  ),
  magnitude_real = list(
    data_type = "Total chlorophyll-a produced during the spring phytoplankton bloom period",
    units = "days*mg/m3"
  ),
  annual_mean = list(
    data_type = "Average chlorophyll-a over the year",
    units = "mg/m3"
  ),
  NRMSE_bloom = list(
    data_type = "Root mean square error between the fitted Gaussian and real chl-a values during the bloom period, normalized to amplitude_real",
    units = "mg/m3"
  ),
  percent_dineof = list(
    data_type = "Percentage of days with pixel values estimated using DINEOF",
    units = ""
  )
)
...
dfsa <- df %>%
  ...
  dplyr::mutate(
    amplitude_real_climmean = mean(amplitude_real, na.rm = TRUE),
  ...
  dplyr::ungroup() %>%
  dplyr::mutate(
    amplitude_real_sanom = (amplitude_real - amplitude_real_climmean) / amplitude_real_climsd,
...
for (metric in metrics) {
  ...
  r <- lapply(times, function(ti) {
    ...
    do.call(what = c) %>%
    setNames(paste0(names(.), "_", year(as_date(times))))
  time(r) <- times
