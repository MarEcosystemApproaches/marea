...
- `ea_spatial`: gridded or spatial/time-varying fields (e.g. maps of temperature)
...
The style argument can be specified to return different plot types, this can
provide a range of reasonable defaults for different data sets.


Other styles: `"default"`, `"plain"`, `"anomaly"`, etc.

These are ggplot objects and are easy to add on to with the `+` syntax (See advanced examples)
...
The style argument can be used to specify some reasonable spatial default styles.
There are also many other arguments for specifying details about coastline objects, regions, and other considerations. See `?plot.ea_spatial` for more details.
Other options:
...
The `ea_spatial()` constructor creates spatial objects from sf data. Here's how to build one:
...
halifax_temp@data
```
This method is direct and explicit but requires you to know the exact slot names. To get a single metadata item, you would need to chain operators, like `halifax_temp@meta$region`.
...
This approach is generally recommended as it provides a single, intuitive interface for retrieving almost any piece of information from an ea_data object without needing to remember the underlying @ syntax.
...
Using robust S4 classes brings multiple benefits:
...
*This vignette was generated using the `marea` source code (2025-07-24).*
