...
<<<<<<< HEAD
An R package of Maritime ecosystem information to help facilitate ecosystem
approaches.
=======
**UNDER DEVELOPMENT**

An R package of Maritime ecosystem information to help facilitate an ecosystem
approach to fisheries management.
>>>>>>> 2d4ff3525ec6414a54cec5742c4ae3ac735b5ba5

*We wrangle the data so you don't have to*

marea stands for MARitimes Ecosystem Approach.

marea is an R package containing a variety of data
sets and model outputs. We wrangle the ecosystem information behind the scenes 
to get them into usable formats in R, and provide helpful plotting functions. 
All data sets include documentation regarding the original sources and authors, 
...
[pacea](https://github.com/pbs-assess/pacea?tab=readme-ov-file#pacea-) which similarly contains Pacific
ecosystem information. pacea in turn was motivated the
[GSLea](https://github.com/duplisea/gslea) R package developed by Dan Duplisea and colleagues for the Gulf of St Lawrence.

## What is in marea?

Currently, marea contains (though it is still in development and help files and
vignettes have not been written yet)::
...
Plotting and helper functions are provided (TODO!), and demonstrated in the detailed vignettes. All
...
**How does the bottom temperature estimated from GLORYS differ between February
and October in 2015 and in 2020?**

```{r glorystempfig, eval = FALSE, echo = FALSE}
png(paste0(here::here(), "/man/figures/README-glorys-temp.png"),
...
```{r glorystemp, eval = FALSE}
plot(glorys_bottom_temperature,
     months = c("February", "October"),
     years = c(2015, 2020))
```

![](man/figures/README-glorys-temp.png){width=80%}

**How does the North Atlantic Oscillation index look through time?**

```{r naofig, eval = FALSE, echo = FALSE}
png(paste0(here::here(), "/man/figures/README-nao.png"),
    width = 670,   # 670 matches size from running from a chunk
    height = 480)
plot(nao,
     lwd = 4)
...
![](man/figures/README-nao.png){width=80%}

TODO need to look into as clearly not normalised.

**How has the estimated abundance of grey seals (TODO give area covered) changed
through time?**

```{r greysealsfig, eval = FALSE, echo = FALSE}
png(paste0(here::here(), "/man/figures/README-grey-seals.png"),
    width = 670,   # 670 matches size from running from a chunk
    height = 480)
plot(grey_seals)
dev.off()
...
```{r greyseals, eval = FALSE}
plot(grey_seals)
```

![](man/figures/README-grey-seals.png){width=80%}
...
update them when the data in marea are updated, so if you run them locally you
...
or contact ???TODO <a href="mailto:andrew.edwards@dfo-mpo.gc.ca">Andy</a>.

## Updates

We plan to continually enhance `marea` by updating current data sets and adding
new ones, as well as adding functionality. To explain these, we will document them
by date in the [NEWS](NEWS.md).

TODO We plan to update the indices, buoy temperatures, and OISST values on the 20th of every month (or soon
after), which should capture updates to those data.

To check what date you installed `marea` locally
...
`marea` by running `remotes::install_github("MarEcosystemApproaches/marea")` again.
...
A comprehensive [2022 analysis of Canadian stock assessments](https://publications.gc.ca/collections/collection_2022/mpo-dfo/Fs97-6-3473-eng.pdf) found that availability of data on environmental variables was the leading cause of not integrating such information into assessments. marea aims to help make data availability more streamlined, and avoid each individual assessment scientist having to figure out themselves where to get appropriate data.

This work is strongly motivated by, and based on, the
[pacea](https://github.com/pbs-assess/pacea?tab=readme-ov-file#pacea-) R package by Andrew Edwards and
colleagues for Pacific ecosystem information, which in turn was inspired by the
[GSLea](https://github.com/duplisea/gslea) R package by Dan Duplisea and colleagues for the Gulf of St Lawrence.

## Citation

If you use `marea` in your work then please cite it as (NOTE this has been
...
TODO: You may wish to add the date you installed it (using `marea_installed()`). Use
`citation("marea")` to get a version for LaTeX and R Markdown
...
maintaining `marea`. <!-- For pacea this hasn't really happened unless we happen
to talk to people! -->
...
1. TODO for marea: Logo included by running `use_logo("inst/pacea-logo-from-carley.png")`. The .png was
...
Your own branch should be called `dev-yourname`, e.g. `dev-jamie`. If you don't
yet have a branch you need to:
...
found these all take a long time for pacea (and it's easier to push and let GitHub check
the coverage). See pacea commit 6c872da for the commands Andy tried (now deleting to
